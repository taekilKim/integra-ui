...
export default function DialogDocsPage() {
  ...
  const behaviorProps = [
    { property: "showCloseIcon", type: "boolean", default: "true", description: "우측 상단 닫기 아이콘 표시 여부" },
    { property: "showSubtext", type: "boolean", default: "true", description: "DialogDescription(서브텍스트) 표시 여부" },
    { property: "showBody", type: "boolean", default: "true", description: "DialogBody(다이얼로그 바디) 표시 여부" },
  ];
...
import { ButtonGroup } from "@/components/ui/button-group"`;
  ...
  >
    ...
    <DialogBody>
      You can place form fields and supporting content in this body area.
    </DialogBody>
    <DialogFooter>
      <ButtonGroup layout="single-fill">
  ...
  return (
    <div className="space-y-64 pb-120">
      ...
      <section className="space-y-24">
        <div className="space-y-8">
          <h2 className="fs-24 font-bold tracking--2 leading-32 text-integra-gray-900">Behavior Props</h2>
          <p className="fs-16 text-integra-gray-500">닫기 아이콘, 서브텍스트, 다이얼로그 바디 가리기 여부를 프로퍼티로 제어할 수 있습니다.</p>
        </div>
        <div className="rounded-12 border border-integra-gray-100 overflow-hidden shadow-sm">
          <table className="w-full text-left border-collapse">
            <thead className="bg-integra-gray-50 border-b border-integra-gray-200">
              <tr className="fs-12 font-bold text-integra-gray-400">
                ...
                <th className="px-24 py-16">Type</th>
                <th className="px-24 py-16">Default</th>
                <th className="px-24 py-16">Description</th>
              </tr>
            </thead>
            <tbody className="divide-y border-integra-gray-100 fs-14">
              {behaviorProps.map((prop) => (
                <tr key={prop.property} className="hover:bg-integra-gray-50/50 transition-colors">
                  <td className="px-24 py-16 font-mono text-integra-blue-600 font-bold">{prop.property}</td>
                  <td className="px-24 py-16 font-mono text-integra-gray-700">{prop.type}</td>
                  <td className="px-24 py-16 font-mono text-integra-gray-700">{prop.default}</td>
                  <td className="px-24 py-16 text-integra-gray-700">{prop.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* 5. 아토믹 설계 원리 설명: bg-integra-gray-50 적용 */}
